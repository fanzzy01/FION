-- // FionHub UI Library (Core Engine) - Optimized for Fion
...
local ShowCursorBinding = "FionCursorUpdate"  -- Nama binding untuk RenderStep
...
Library._ScreenGui = Library._ScreenGui
...
function Library:Unload()
    ...
    for _, c in ipairs(Library._Conns or {}) do
        pcall(function() c:Disconnect() end)
    end
    table.clear(Library._Conns)
    if _G.FionHub_WMConn then
        pcall(function() _G.FionHub_WMConn:Disconnect() end)
        _G.FionHub_WMConn = nil
    end
    if _G.FionHub_BarConn then
        pcall(function() _G.FionHub_BarConn:Disconnect() end)
        _G.FionHub_BarConn = nil
    end
    pcall(function()
...
Library.Watermark = {
    ...
    ShowPing = true,
    Text = "FionHub",
    Icon = "rbxassetid://101468688188732",
}
...
function Library:SafeCallback(fn, ...)
    ...
    if not ok then warn("[FionHub callback]", err) end
end
...
function Library:Notify(options)
    ...
    Tween(TitleText, {TextTransparency = 0}, 0.2)
    Tween(DescText, {TextTransparency = 0}, 0.2)
...
pcall(function()
    local function wipe(parent)
        ...
        for _, gui in ipairs(parent:GetChildren()) do
            ...
            if string.find(n, "FionHub_UI") or string.find(n, "FionHub") then
                gui:Destroy()
            end
        end
    end
...
function Library:SetWatermark(opts)
    ...
    Library.Watermark = Library.Watermark or {}
    if opts.Enabled ~= nil then Library.Watermark.Enabled = opts.Enabled end
    if opts.ShowTime ~= nil then Library.Watermark.ShowTime = opts.ShowTime end
    if opts.ShowFPS ~= nil then Library.Watermark.ShowFPS = opts.ShowFPS end
    if opts.ShowPing ~= nil then Library.Watermark.ShowPing = opts.ShowPing end
    if opts.Text ~= nil then Library.Watermark.Text = tostring(opts.Text) end
    if opts.Icon ~= nil then Library.Watermark.Icon = tostring(opts.Icon) end
    if Library._UpdateWatermark then
        Library._UpdateWatermark(true)
    end
end
...
function Library:CreateWindow(options)
    ...
    local hubName = "FionHub"
    local subText = "Made by FionHub"
...
local MAX_W, MAX_H = 900, 560
    ...
    if type(options) == "table" then
        ...
        if typeof(options.Size) == "UDim2" then
            WIN_W = math.clamp(options.Size.X.Offset, MIN_W, MAX_W)
            WIN_H = math.clamp(options.Size.Y.Offset, MIN_H, MAX_H)
        end
        
        Library.Watermark.Enabled = options.Watermark == true
        if type(options.WatermarkText) == "string" then
            Library.Watermark.Text = options.WatermarkText
        end
        
        if type(options.WatermarkIcon) == "string" then
            Library.Watermark.Icon = options.WatermarkIcon
        end
        
        if options.SidebarCompacted == true then
            SIDE_W = 52
        end
...
local ModalElement = Create("TextButton", {
    ...
    -- ===== WATERMARK =====
    local WM = Library.Watermark or {}
    local WatermarkHold = Create("Frame", {
        Parent = ScreenGui,
        BackgroundTransparency = 1,
        ...
        ZIndex = 180,
        Active = true,
        Visible = WM.Enabled == true,
    })

    local CenterPill = Create("Frame", {
        Parent = WatermarkHold,
    ...
    Create("UICorner", {Parent = CenterPill, CornerRadius = UDim.new(1, 0)})
    Create("UIStroke", {
        Parent = CenterPill,
    ...
    local HomeIcon = Create("ImageLabel", {
        ...
        Image = (Library.Watermark and Library.Watermark.Icon) or "rbxassetid://101468688188732",
    ...
    local CenterText = Create("TextLabel", {
        ...
        TextXAlignment = Enum.TextXAlignment.Left,
        Text = "00:00:00  |  60 FPS  |  0ms",
        ZIndex = 182,
    })

    MakeDraggable(CenterPill, WatermarkHold)

    local wmFrames, wmLast, wmFPS = 0, tick(), 60
    local wmConn
    ...
    local function UpdateWatermark(force)
        local on = Library.Watermark and Library.Watermark.Enabled == true
        WatermarkHold.Visible = on
        if HomeIcon then
            HomeIcon.Image = (Library.Watermark.Icon and Library.Watermark.Icon ~= "")
                and Library.Watermark.Icon
                or "rbxassetid://101468688188732"
        end
        if not on then return end

        local now = tick()
        wmFrames += 1
        if not force and now - wmLast < 0.5 then return end
        wmFPS = math.floor(wmFrames / math.max(now - wmLast, 0.001) + 0.5)
        wmFrames = 0
        wmLast = now

        local parts = {}
        if Library.Watermark.Text and Library.Watermark.Text ~= "" then
            table.insert(parts, tostring(Library.Watermark.Text))
        end
        if Library.Watermark.ShowTime ~= false then
            table.insert(parts, os.date("%H:%M:%S"))
        end
        if Library.Watermark.ShowFPS ~= false then
            table.insert(parts, tostring(wmFPS) .. " FPS")
        end
        if Library.Watermark.ShowPing ~= false then
            local ping = 0
            pcall(function()
        ...
        if mid == "" then mid = "FionHub" end
        CenterText.Text = mid
        FitPill(mid)
    end

    Library._UpdateWatermark = UpdateWatermark
    UpdateWatermark(true)

    if _G.FionHub_WMConn then
        pcall(function() _G.FionHub_WMConn:Disconnect() end)
    end
    wmConn = Track(RunService.Heartbeat:Connect(function()
    UpdateWatermark(false)
end))
_G.FionHub_WMConn = wmConn
    ...
    local function SendPremiumNotification()
        ...
        local LockIcon = Create("ImageLabel", {Parent = Notif, BackgroundTransparency = 1, Size = UDim2.new(0, 24, 0, 24), Position = UDim2.new(0, 15, 0.5, -12), Image = "rbxassetid://84662895793846", ImageColor3 = Color3.fromRGB(255, 215, 0), ImageTransparency = 1, ZIndex = 202})
...
ScreenGui.Destroying:Connect(function()
    ...
    local TitleContainer = Create("Frame", {
        Parent = TopBar,
        BackgroundTransparency = 1,
        Size = UDim2.new(0, SIDE_W <= 60 and 110 or 160, 1, 0),
...
local titleReserve = (topbarLogo and (8 + logoSize + 8) or 15) + 110
local searchW = SIDE_W <= 60 and 170 or 220
...
local SearchBar = Create("Frame", {
    ...
    local SearchIcon = Create("ImageLabel", {Parent = SearchBar, BackgroundTransparency = 1, Image = "rbxassetid://84662895793846", ImageColor3 = SubTextColor, Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(0, 8, 0.5, -7)})
    ...
    Parent = Sidebar,
        BackgroundTransparency = 1,
        Size = UDim2.new(1, SIDE_W <= 60 and -8 or -15, 1, SIDE_W <= 60 and -10 or -94),
        Position = UDim2.new(0, SIDE_W <= 60 and 4 or 10, 0, SIDE_W <= 60 and 6 or 40),
...
local restoringCursor = false

local function RestoreMouseIcon()
    if restoringCursor then return end
...
local function SetCustomCursor(on)
    ...
    if on then
        ...
        if not MainFrame or not MainFrame.Parent or not MainFrame.Visible then
            return
        end

        if ModalElement then ModalElement.Modal = true end
        if CustomCursor then
            CustomCursor.Visible = Library.ShowCustomCursor == true
        end
...
if isMobileDevice then
    ...
    function Window:CreateTab(nameOrConfig, iconOrDefault, locked)
    local tabName, tabIcon, isDefault, isLocked

    if type(nameOrConfig) == "table" then
        tabName   = nameOrConfig.Title or nameOrConfig.Name or "Tab"
        tabIcon   = nameOrConfig.Icon
        isDefault = nameOrConfig.Default == true
        isLocked  = nameOrConfig.Locked == true
    else
        tabName   = tostring(nameOrConfig or "Tab")
        tabIcon   = typeof(iconOrDefault) == "string" and iconOrDefault or nil
        isDefault = iconOrDefault == true
        isLocked  = locked == true
    end
    local compact = SIDE_W <= 60
        ...
        local TabIcon = Create("ImageLabel", {
            ...
            BackgroundTransparency = 1,
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = compact and UDim2.new(0.5, 0, 0.5, 0) or UDim2.new(0, 18, 0.5, 0),
        ...
        local Txt = Create("TextLabel", {
            ...
            TextColor3 = SubTextColor,
            BackgroundTransparency = 1,
            Size = compact and UDim2.new(0, 0, 0, 0) or UDim2.new(1, tabIcon and -50 or -40, 1, 0),
            Position = compact and UDim2.new(0, 0, 0, 0) or UDim2.new(0, tabIcon and 34 or 15, 0, 0),
            TextXAlignment = Enum.TextXAlignment.Left,
            Visible = not compact,
        })

        if tabIcon then
            Library:ApplyIcon(TabIcon, tabIcon, TextColor)
        end

        if compact then
            Txt.Visible = false
            TabIcon.Visible = tabIcon ~= nil
            TabBtn.Size = UDim2.new(1, 0, 0, 36)
        end

        if isLocked then
            Create("ImageLabel", {Parent = TabBtn, Image = "rbxassetid://84662895793846", ImageColor3 = Color3.fromRGB(255, 215, 0), BackgroundTransparency = 1, Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(1, -22, 0.5, -7)})
        end
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            for _, other in ipairs(Window.Tabs) do
                ...
                if other.Txt then Tween(other.Txt, {TextColor3 = SubTextColor}, 0.15) end
            end

            Window.CurrentTab = TabConfig
            TabConfig.Content.Visible = true
            TabConfig.Content.Position = UDim2.new(0, 0, 0, 0)
            ...
            if #TabConfig.Pages > 0 then
                ...
                if TabConfig.CurrentPage ~= firstPage then
                    ...
                    firstPage.Scroll.Position = UDim2.new(0, 5, 0, 5)
...
local R_Layout = Create("UIListLayout", {Parent = RightColumn, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 10)})
    ...
    TextColor3 = TextColor,
    BackgroundTransparency = 1,
    Size = UDim2.new(1, reserveIcon and -56 or -36, 0, 30),
...
local Pad = Create("UIPadding", {Parent = ItemContainer, PaddingBottom = UDim.new(0, 10), PaddingTop = UDim.new(0, 8)})
...
local function SetSectionOpen(open)
    sectionOpen = open
    ItemContainer.Visible = open
    CollapseBtn.Text = open and "˅" or "˄"
    SectionContainer.AutomaticSize = open and Enum.AutomaticSize.Y or Enum.AutomaticSize.None
    if not open then
...
CollapseBtn.MouseButton1Click:Connect(function()
    SetSectionOpen(not sectionOpen)
end)
...
Chip.Size = UDim2.new(0, 44, 0, 16)
                        ...
                        UserInputService.InputBegan:Connect(function(input, gpe)
                            ...
                            if listening then
                                ...
                                bind = input.KeyCode
                                Chip.Text = KeyName(bind)
                                Library:SafeCallback(keyCb, KeyName(bind))
                                return
                            end
                            if bind and not gpe and input.KeyCode == bind then
                                internalSet(not state)
                                Library:SafeCallback(keyCb, state)
                            end
                        end)
...
Library.Options[colorFlag] = { Type = "ColorPicker", Value = color, SetValue = internalSet }
                ...
                function Elements:AddConfigManager(folderName)
                    folderName = folderName or "FionHubSavers"
